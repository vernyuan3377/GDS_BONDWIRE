...
def _hfss_script(wires: list[dict]) -> str:
    lines = [
        "# Run from Ansys Electronics Desktop: Tools > Run Script",
        "import ScriptEnv",
        'ScriptEnv.Initialize("Ansoft.ElectronicsDesktop")',
        "oDesktop.RestoreWindow()",
        "oProject = oDesktop.GetActiveProject()",
        'oDesign = oProject.GetActiveDesign() if oProject else None',
        'oEditor = oDesign.SetActiveEditor("3D Modeler") if oDesign else None',
        'assert oEditor is not None, "Open an HFSS 3D design before running this script."',
        "",
    ]
    for wire in wires:
        point_rows = []
        for x, y, z in wire["points_mil"]:
            point_rows.append(
                f'["NAME:PLPoint","X:=","{x:.9f}mil","Y:=","{y:.9f}mil","Z:=","{z:.9f}mil"]'
            )
        points = ",".join(point_rows)
        segments = ",".join(
            f'["NAME:PLSegment","SegmentType:=","Line","StartIndex:=",{index},"NoOfPoints:=",2]'
            for index in range(len(wire["points_mil"]) - 1)
        )
        lines.extend(
            [
                "oEditor.CreatePolyline(",
                '  ["NAME:PolylineParameters","IsPolylineCovered:=",True,"IsPolylineClosed:=",False,',
                f'   ["NAME:PolylinePoints",{points}],',
                f'   ["NAME:PolylineSegments",{segments}],',
                '   ["NAME:PolylineXSection","XSectionType:=","Circle","XSectionOrient:=","Auto",',
                f'    "XSectionWidth:=","{wire["diameter_um"]:.6f}um","XSectionTopWidth:=","0um",',
                '    "XSectionHeight:=","0um","XSectionNumSegments:=","12","XSectionBendType:=","Curved"]],',
                f'  ["NAME:Attributes","Name:=","BondWire_{wire["index"]}","Flags:=","","Color:=","(255 180 0)",',
                '   "Transparency:=",0,"PartCoordinateSystem:=","Global","UDMId:=","",',
                '   "MaterialValue:=","\\"gold\\"","SurfaceMaterialValue:=","\\"\\"",',
                '   "SolveInside:=",False,"ShellElement:=",False,"ShellElementThickness:=","0mm",',
...
def export_simulation_package(
    directory: str | Path,
    bonds: list[Bond],
    endpoints: Callable[[Bond], tuple[Point3D, Point3D]],
    labels: Callable[[Bond, str], str],
) -> list[Path]:
    ...
    for index, bond in enumerate(bonds, 1):
        ...
        cell.add(
            gdstk.FlexPath(
                [(x * UM_PER_MIL, y * UM_PER_MIL) for x, y, _ in points],
                diameter_um,
                layer=200,
                datatype=index,
            )
        ...
        wire = {
            "index": index,
            "endpoint_a": labels(bond, "chip"),
            "endpoint_b": labels(bond, "board"),
            "start_mil": start,
            "control_mil": control,
            "end_mil": end,
            "diameter_um": diameter_um,
            "length_mil": quadratic_length(start, control, end),
            "points_mil": points,
        }
        wires.append(wire)
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as stream:
        ...
        writer.writerow(
            ["wire", "point", "x_mil", "y_mil", "z_mil", "x_um", "y_um", "z_um", "diameter_um", "length_mil"]
        )
        for wire in wires:
            for point_index, (x, y, z) in enumerate(wire["points_mil"]):
                writer.writerow(
                    [
                        ...
                        f"{y * UM_PER_MIL:.9f}",
                        f"{z * UM_PER_MIL:.9f}",
                        f"{wire['diameter_um']:.6f}",
                        f"{wire['length_mil']:.9f}",
                    ]
                )
    ...
    readme_path.write_text(
        "layout.gds contains the 2D bondwire projection for ADS/HFSS layout import.\n"
