...
- 使用独立的“PCB PAD 生成与编辑”窗口绘制手工 PAD，可设置任意编号、位置、尺寸、旋转、形状和颜色。
- PAD 编号支持负号（如 `-1`、`PAD-01`）；批量生成和多选编辑支持勾选 `ΔPAD` 编号递增/递减、`ΔX` 和 `ΔY` 坐标步进，支持 mil/mm 单位。
- PAD 编号支持负号和跨零步进，例如 `PAD-01`、`PAD-02`，或 `-1`、`0`、`1`。
- “生成单个 PAD”每次只创建当前 PAD，不受批量数量影响；启用步进后会自动准备下一组编号和坐标，便于连续点击生成。
- 手工 PAD 支持鼠标拖动、坐标编辑、左/右/上/下对齐和水平/垂直等距分布；等距分布可使用自动间距或指定固定间距。
- 支持全局 `Ctrl+Z` 撤销 PAD、BondWire、芯片/PCB 变换、3D 参数和芯片显示状态；焦点位于 PAD 或 3D 窗口时同样有效。
- 芯片 PAD 标签保持屏幕正向，不随芯片旋转。
- 工具栏可隐藏或显示芯片图形、PAD 和标签，隐藏时保留全部 BondWire 连接数据。
...
4. 如需自定义封装，点击“生成 PAD”打开独立编辑器；设置数量，并按需勾选 `ΔPAD`、`ΔX`、`ΔY`。负的 `ΔPAD` 表示编号递减。
5. 选择“编辑所选 PAD”可修改或拖动画布中的手工 PAD，并可批量对齐或等距分布；切换 BondWire 模式不会破坏已有 PAD 与打线关系。
6. 切换到“绘制 BondWire”，依次点击芯片端点和 PCB 端点。
7. 拖动连线两端微调落点。
